...
export const ToastModal = ({
  ...
  return createPortal(
    ...
    <div className={`fixed z-[60] flex transition-opacity duration-300 ${isSuccess
      ? "top-28 right-4 sm:top-44 sm:right-10 pointer-events-none"
      ...
      <div
        ...
        ${isSuccess
            ? "w-auto px-5 py-3 sm:px-6 sm:py-4 rounded-xl sm:rounded-2xl flex items-center gap-3 pointer-events-auto scale-100 border border-green-100"
      ...
      >
        ...
        <div className={`flex items-center gap-3 ${!isSuccess ? "mb-4 sm:mb-6" : ""}`}>
          ...
          <div className={`flex-shrink-0 flex items-center justify-center border ${isSuccess
            ? "w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-green-50 text-green-500 border-green-300"
          ...
          {/* Alert message */}
          <p className="text-gray-700 text-sm sm:text-base font-semibold leading-snug whitespace-nowrap">
            {toast.message}
          </p>
        </div>
