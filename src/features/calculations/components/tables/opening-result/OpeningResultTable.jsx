...
// Maps the internal opening type key to a human-readable description
const typeLabelMap = {
  box: "Box Type Opening",
  r: "R Type Opening",
};
...
//
// IMPORTANT: when `value` is missing (null/undefined/"-"), we do NOT fall
// back to "NG". A missing safety factor means the check hasn't produced a
// number at all — showing "NG" there would tell the user something failed
// structurally when really the data just isn't there yet. So this now has
// three distinct states: OK, NG, and "No Data".
...
// Component to display opening calculation results in a table format
export function OpeningResultTable({ openingType }) {
  const limitValue = openingType?.limit ?? 1.0;

  const rows = [
    {
      no: 1,
      description: typeLabelMap[openingType?.type] ?? "-",
      safetyFactor: openingType?.safetyFactor ?? "-",
      limit: limitValue,
    },
  ];
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mt-12">
      ...
      <div className="px-4 md:px-6 py-4 md:py-5 flex items-center gap-3 border-b border-gray-200">
